...
def call_pruning_metrics(base_train_dataset, test_loader, config, device):
    ...
    if config.pruning_method >= 100 and config.pruning_method < 200:
        ...
        loss_all_epoch_raw = np.load('/home/local/zi/research_project/quality_air/VLM_training_dynamics.npy')
        # 删除所有全 0 列
        loss_all_epoch = loss_all_epoch_raw[:, ~(loss_all_epoch_raw == 0).all(axis=0)]
        # 过滤掉可能是全0行(一般是无用行)
        mask = np.any(loss_all_epoch[:, 1:] != 0, axis=1)
    ...
    #     /home/zi/research_project/regression/quality_air/weather_itrans_loss_all_epoch_seed_0.npy

    elif config.pruning_method >= 700 and config.pruning_method < 800:
        # weather_all_token (36696, 11, 96, 21)
        loss_all_epoch_raw = np.load("/home/local/zi/research_project/iTransformer/"
                                     "all_saved_seed0_trained_per_token_loss/"
                                     "all_saved_seed0_pm0_pr0_low10_high10_start0_int20_"
                                     "test1_iTransformer_custom_ftM_sl96_ll48_pl96_"
    ...
    elif config.pruning_method >= 800 and config.pruning_method < 900:
        # weather unique token (36696, 11, 96, 21)
        loss_all_epoch = np.load("/home/local/zi/research_project/iTransformer/"
                                     "all_saved_seed0_trained_per_token_loss/"
                                     "all_saved_seed0_pm0_pr0_low10_high10_start0_int20_"
                                     "test1_iTransformer_custom_ftM_sl96_ll48_pl96_"
                                     "dm512_nh8_el3_dl1_df512_fc1_ebtimeF_dtTrue_"
        ...
        num_samples, num_epochs, forecast_steps, num_vars = loss_all_epoch.shape  # 36696, 11, 96, 21

        # --- 第一步：重排 A ---
        # 原始 loss_all_epoch 的轴含义： (样本, epoch, 预测步长, 变量)
        # 需要将样本和预测步长合并，得到 B 的轴： (样本*预测步长, epoch, 变量)
        B = loss_all_epoch.transpose(0, 2, 1, 3).reshape(num_samples * forecast_steps, num_epochs, num_vars)
        # 现在 B 的 shape 为 (36696*96, 11, 21)

        # --- 第二步：生成时间 index ---
        # 对于第 i 个样本（起始时间 i）和第 j 个预测步，其真实时间 = i + j
        time_index = (np.arange(num_samples)[:, None] + np.arange(forecast_steps)[None, :]).reshape(-1)
        # time_index 的 shape 为 (36696*96,)，例如：
        # 第一行（样本0）：0, 1, 2, …, 95
        # 第二行（样本1）：1, 2, 3, …, 96
        # … 依此类推

        # --- 第三步：对相同 time_index 的 B 行求均值 ---
        # 为了提高效率，先对 time_index 进行排序，同时重排 B
        order = np.argsort(time_index)
        time_index_sorted = time_index[order]
        B_sorted = B[order]

        # 利用 np.unique 找到所有唯一的时间值及每组的起始位置和计数
        unique_time, start_indices, counts = np.unique(time_index_sorted, return_index=True, return_counts=True)

        # 利用 np.add.reduceat 在排序后的 B 上对各组进行求和
        group_sums = np.add.reduceat(B_sorted, start_indices, axis=0)
        # 计算均值：每组和除以该组样本数（利用广播）
        mean_B = group_sums / counts[:, None, None]
        # 最终 mean_B 的 shape 为 (len(unique_time), 11, 21)
        # unique_time 数组保存了每个均值对应的真实时间 index

        # 交换最后两个轴，将 (time, epoch, variable) -> (time, variable, epoch)
        mean_B = mean_B.transpose(0, 2, 1)  # shape 变为 (36791, 21, 11)

        # 将前两个轴合并，得到每个变量在每个时间点为一行，列为不同 epoch 的矩阵
        loss_all_epoch = mean_B.reshape(-1, mean_B.shape[-1])
    ...
    if ((config.pruning_method % 100 == 0)
    ...
    elif (config.pruning_method % 100 == 6):
        avg_loss_matrix = loss_all_epoch[:, :2].copy()  # shape=(?,2)
        avg_loss_matrix[:, 1] = loss_all_epoch[:, 1:].var(axis=1)
        sorted_metrics = avg_loss_matrix[np.argsort(avg_loss_matrix[:, 1])]
    ...
    elif (config.pruning_method % 100 == 5):
        # Avg Loss Changes
        ...
        avg_loss_changes_matrix = loss_differences[:, :2].copy()  # shape=(?,2)
        avg_loss_changes_matrix[:, 1] = loss_differences[:, 1:].mean(axis=1)
        sorted_metrics = avg_loss_changes_matrix[np.argsort(avg_loss_changes_matrix[:, 1])]
...
def main():
    ...
    pm_names = {
        ...
        102: "TTDS",
        103: "Avg Loss",
        104: "Var Loss Changes (TTDS)",
        105: "Avg Loss Changes",
        106: "Var Loss",
        ...
        202: "TTDS",
        203: "Avg Loss",
        204: "Var Loss Changes (TTDS)",
        205: "Avg Loss Changes",
        206: "Var Loss",
        ...
        302: "TTDS",
        303: "Avg Loss",
        304: "Var Loss Changes (TTDS)",
        305: "Avg Loss Changes",
        306: "Var Loss",
        ...
        402: "TTDS",
        403: "Avg Loss",
        404: "Var Loss Changes (TTDS)",
        405: "Avg Loss Changes",
        406: "Var Loss",
        ...
        502: "TTDS",
        503: "Avg Loss",
        504: "Var Loss Changes (TTDS)",
        505: "Avg Loss Changes",
        506: "Var Loss",
        ...
        602: "TTDS",
        603: "Avg Loss",
        604: "Var Loss Changes (TTDS)",
        605: "Avg Loss Changes",
        606: "Var Loss",
        ...
        702: "TTDS",
        703: "Avg Loss",
        704: "Var Loss Changes (TTDS)",
        705: "Avg Loss Changes",
        706: "Var Loss",
        ...
        802: "TTDS",
        803: "Avg Loss",
        804: "Var Loss Changes (TTDS)",
        805: "Avg Loss Changes",
        806: "Var Loss",
    }
    # [-1, 1, 3, 4, 101, 103, 104]
    ...
    if config.pruning_method < 300:
        ...
        # 7344 'NMHC(GT)' 很多NAN
    elif (config.pruning_method >= 300) and (config.pruning_method < 1700):
    ...
    if config.pruning_method < 100:
        ...
        dataset_name = 'Weather(all tokens)'
    elif config.pruning_method < 900:
        dataset_name = 'Weather(unique tokens)'
    ...
    if plot_metric_distribution:
        if (config.pruning_method % 100 == 3
                ...
                or config.pruning_method % 100 == 5
                or config.pruning_method % 100 == 6):
            ...
            high_percent = 99.0
            # 计算1%和99%分位数
            lower_bound = np.percentile(metric_values, low_percent)
            upper_bound = np.percentile(metric_values, high_percent)
            ...
            plt.title(
                f'Histogram of {pm_names[config.pruning_method]} on {dataset_name}',
                fontsize=14)
            ...
            file_path = f'./plot_results/2_{dataset_name}_{pm_names[config.pruning_method]}_metric_distribution.png'
            os.makedirs(os.path.dirname(file_path), exist_ok=True)
            plt.savefig(file_path)
            plt.close('all')

            #
            # metric_values = np.var(loss_all_epoch[:,1:], axis=1)
            # # 计算1%和99%分位数
            # lower_bound = np.percentile(metric_values, low_percent)
            # upper_bound = np.percentile(metric_values, high_percent)
            # metric_values = metric_values[(metric_values >= lower_bound) & (metric_values <= upper_bound)]
            ...
            # plt.ylabel('Frequency', fontsize=12)
            # plt.title(
            #     f'Histogram of variance of loss curves on {dataset_name}',
            #     fontsize=14)
            # plt.grid(axis='y', linestyle='--', alpha=0.6, color='black')
            #
            # # 适合论文风格的去除边框
            # plt.gca().spines['top'].set_visible(False)
            # plt.gca().spines['right'].set_visible(False)
            # # file_path = f'./plot_results/{pm_names[config.pruning_method]}/2_{dataset_name}_Var Loss_metric_distribution.png'
            # file_path = f'./plot_results/2_{dataset_name}_Var Loss_metric_distribution.png'
            # os.makedirs(os.path.dirname(file_path), exist_ok=True)
            # plt.savefig(file_path)
            # plt.close('all')
            #
            # num_epochs = loss_all_epoch.shape[1] - 1
            # loss_differences = np.zeros((loss_all_epoch.shape[0], num_epochs + 1))
            # loss_differences[:, 0] = loss_all_epoch[:, 0]
            #
            # for ep_i in range(1, num_epochs + 1):
            #     if ep_i == 1:
            ...
            #     loss_differences[:, ep_i] = diff
            #
            # metric_values = np.mean(loss_differences[:, 1:], axis=1)
            # # 计算1%和99%分位数
            # lower_bound = np.percentile(metric_values, low_percent)
            # upper_bound = np.percentile(metric_values, high_percent)
            # # 过滤数据，去除前5%和后5%的值
            ...
            # plt.ylabel('Frequency', fontsize=12)
            # plt.title(
            #     f'Histogram of average loss changes on {dataset_name}',
            #     fontsize=14)
            # plt.grid(axis='y', linestyle='--', alpha=0.6, color='black')
            #
            # # 适合论文风格的去除边框
            # plt.gca().spines['top'].set_visible(False)
            # plt.gca().spines['right'].set_visible(False)
            # file_path = f'./plot_results/2_{dataset_name}_Avg Loss Changes_metric_distribution.png'
            # os.makedirs(os.path.dirname(file_path), exist_ok=True)
            # plt.savefig(file_path)
            # plt.close('all')
    ...
    if plot_loss_curve:
        ...
        ranked_percent = [0.01, 0.25, 0.5, 0.75, 0.99,]
    ...
    plot_S2L_tsne = False
    if plot_S2L_tsne:
        if config.pruning_method % 100 == 1:
            cluster_indices_dict = sorted_metrics['cluster']

            # --------------------------
            # 根据 cluster_indices_dict 建立 sample_ID 到 label 的映射
            # --------------------------
            sample_to_label = {}
            for label, sample_ids in cluster_indices_dict.items():
                for sid in sample_ids:
                    sample_to_label[int(sid)] = label

            # 提取特征：loss_all_epoch 的第1列为 sample_ID，第2~31列为特征
            features = loss_all_epoch[:, 1:]  # shape = (样本数, 30)

            # 使用 t-SNE 将 30 维数据降为 2 维
            tsne = TSNE(n_components=2, random_state=42)
            tsne_result = tsne.fit_transform(features)  # shape = (样本数, 2)

            # 根据 sample_ID 获取对应的 label
            # 注意：loss_all_epoch 的第一列可能是 float 类型，所以转换为 int
            labels = [sample_to_label[int(sid)] for sid in loss_all_epoch[:, 0]]
            sil_score = silhouette_score(features, labels)
            ch_score = calinski_harabasz_score(features, labels)
            # print(f"Calinski-Harabasz Score (CH): {ch_score:.4f}")
            # print(f"Silhouette Score: {sil_score:.4f}")

            # 创建一个空的 Figure
            fig = go.Figure()

            # 为了便于区分，我们对每个簇分别绘制一个 trace
            for label in sorted(cluster_indices_dict.keys()):
                # 找到当前簇对应的样本索引
            ...
            fig.update_layout(
                # title=f'Loss curves of ranked samples based on metric {pm_names[config.pruning_method]}',
                title=dict(
                    text=f't-SNE Visualization of Samples based on S2L on {dataset_name} <br> (Silhouette Score: {sil_score:.4f}, Calinski-Harabasz Score (CH): {int(ch_score)})',
                    font=dict(size=20, family="Arial, sans-serif"),
                    x=0.5,  # Center title
                    y=0.95
                ),
                ...
                font=dict(size=14, family="Arial, sans-serif"),
                xaxis=dict(showgrid=True, zeroline=False, tickmode='linear', dtick=20, title_font=dict(size=16)),
                # xaxis=dict(showgrid=True, zeroline=False, tickmode='linear', title_font=dict(size=16)),
                yaxis=dict(showgrid=True, zeroline=False, title_font=dict(size=16)),
                hovermode="closest",  # Remove vertical hover line
                margin=dict(l=60, r=60, t=60, b=60),
                plot_bgcolor="white",
            ...
            # 将图像保存为 HTML 文件
            # fig.write_html("./tsne_scatter.html")
            # print("散点图已保存为 tsne_scatter.html")
            html_file = f'./plot_results/s2l_tsne_scatter/{dataset_name}_{pm_names[config.pruning_method]}_tsne_scatter.html'
            os.makedirs(os.path.dirname(html_file), exist_ok=True)
