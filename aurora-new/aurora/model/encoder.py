...
class Perceiver3DEncoder(nn.Module):
    ...
    def forward(self, batch: Batch, lead_time: timedelta) -> torch.Tensor:
        ...
        assert set(self.surf_vars) <= set(surf_vars), f"Missing surface variables: {set(self.surf_vars) - set(surf_vars)}."
        assert set(self.atmos_vars) <= set(atmos_vars), f"Missing atmospheric variables: {set(self.atmos_vars) - set(atmos_vars)}."
        if static_vars is not None:
            assert set(self.static_vars) <= set(static_vars), f"Missing static variables: {set(self.static_vars) - set(static_vars)}."

        x_surf = torch.stack([batch.surf_vars[k] for k in self.surf_vars], dim=2)
        x_static = torch.stack([batch.static_vars[k] for k in self.static_vars], dim=2)
        x_atmos = torch.stack([batch.atmos_vars[k] for k in self.atmos_vars], dim=2)
        ...
        if static_vars is None:
            ...
            surf_vars_concat = self.surf_vars
        else:
            ...
            surf_vars_concat = self.surf_vars + self.static_vars
        ...
        x_surf = self.surf_token_embeds(x_surf, surf_vars_concat)  # (B, L, D)
