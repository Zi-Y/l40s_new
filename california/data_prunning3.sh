...
# === 配置部分 ===
SCRIPT_PATH="/home/zi/research_project/california/california_data_pruning_singlePR_new.py"  # 脚本路径
NUM_GPUS=4                     # GPU 数量
MAX_JOBS=80                   # 最大并发进程数
...
# for PRUNING_METHOD in 2 3 4
for PRUNING_METHOD in -1 3 4 0 2 -2
do
    for PRUNING_RATE in "${PRUNING_RATES[@]}"
    do
        # 这里将训练 seed 改为示例区间，请根据需要修改
        for TRAINING_SEED in {0..500}
        do
            ...
            if [ "$PRUNING_METHOD" -eq 0 ]; then
                ...
                WANDB_NAME="TTDS_NoWeight"
            elif [ "$PRUNING_METHOD" -eq -1 ]; then
                WANDB_NAME="fixed_random"
            elif [ "$PRUNING_METHOD" -eq -2 ]; then
                WANDB_NAME="full_random"
            else
                WANDB_NAME="error"
            fi
            ...
            python "$SCRIPT_PATH" \
                ...
                --wandb_project california_housing_pruning_new \
                --seeds 1 \
