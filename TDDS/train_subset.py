...
parser.add_argument('--save_path', type=str, default='./checkpoint/pruned-dataset',
...
parser.add_argument('--pruning_methods', type=int, default='0', help='-1 - random pruning, 0 - avg loss, 1 - S2L, 2 - TTDS')

# parser.add_argument('--random_pruning', action='store_true', help='Enable random pruning')
...
def main():
    ...
    if args.pruning_methods == -1:
        ...
        wandb_name = 'TTDS_NoWeight_'
    elif args.pruning_methods == 4:
        wandb_name = 'TTDS_NoWeight_loss_'
    elif args.pruning_methods == 5:
        wandb_name = 'TTDS_loss_'

    wandb.init(
        project="c-class-iterations-full",  # 替换为你的项目名
        name=wandb_name + f"pr{args.subset_rate}_{args.arch}_{args.dataset}_seed{args.manualSeed}",
        config=vars(args)  # 直接记录args到config
    )
