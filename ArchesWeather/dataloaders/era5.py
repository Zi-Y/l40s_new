...
class NetcdfDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        if '.nc' in path:
            ...
            self.files = sorted([str(x) for x in Path(path).glob('*.nc') if filename_filter(x.name)],
                                key=lambda x: x.replace('6h', '06h').replace('0h','00h'))

        self.nfiles = len(self.files)

        # test_only
        # only for test, reduce the training samples only using 1979 year
        # if self.nfiles > 20:
        ...
        # import pickle
        # # 保存到 pickle 文件
        # with open('/home/zi/research_project/ArchesWeather/sample_list.pkl', 'wb') as file:  # 使用 'wb' 模式写入二进制文件
        #     pickle.dump(self.id2pt, file)
    ...
    def __getitem__(self, i):
        ...
        # import pickle
        # with open("sample_list.pkl", "wb") as tf:
        #     pickle.dump(self.id2pt, tf)
        #
        # import pickle
        # with open("sample_list.pkl", "rb") as tf:
...
class Era5Forecast(NetcdfDataset):
    ...
    def __init__(self, 
        ...
        if domain in ('val', 'test'):
            ...
            # start_time = np.datetime64(f'{year}-12-01T00:00:00')
            # if self.load_prev:
            #     start_time = start_time - self.lead_time_hours*np.timedelta64(1, 'h')
            # end_time = np.datetime64(f'{year+1}-12-01T00:00:00') + self.multistep*self.lead_time_hours*np.timedelta64(1, 'h')
