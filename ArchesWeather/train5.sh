...
do
    ...
    output_dir="/mnt/ssd/zi/4xl40s_ag_1_seed3_avg_loss_keepeasy_${new_value}"
    mkdir -p "$output_dir"
    ...
    python train_hydra.py \
        ...
        module.info_batch.prune_easy=-2 \
...
do
    new_value=$(awk "BEGIN {print $patch_size * 100}")
    # 创建新的文件夹，文件夹名称为 Aurora_training_Patch_size<patch_size>
    output_dir="/mnt/ssd/zi/4xl40s_ag_1_seed3_avg_loss_keephard_${new_value}"
    mkdir -p "$output_dir"
    ...
    python train_hydra.py \
        ...
        module.use_infobatch=True \
        module.path_save_base="$output_dir" \
        module.info_batch.prune_easy=-1 \
        module.info_batch.info_batch_ratio=$patch_size \
        max_steps=320000 \
