...
class Metadata:
    ...
    rollout_step: torch.Tensor = None
    ...
    def __post_init__(self):
        ...
        if self.rollout_step is None:
            batch_size = len(self.time)
            self.rollout_step = torch.zeros(batch_size, dtype=torch.int32, device=self.lat.device)
...
class Batch:
    ...
    def normalise(self, stats: dict[str, tuple[float, float]]) -> "Batch":
    ...
    def unnormalise(self, stats: dict[str, tuple[float, float]]) -> "Batch":
    ...
    def __getitem__(self, idx: int) -> "Batch":
        time_idx = (self.metadata.time[idx], )
        rollout_step = self.metadata.rollout_step[idx:(idx + 1)]

        return Batch(
            surf_vars={k: v[idx:(idx + 1)] for k, v in self.surf_vars.items()},
            static_vars={k: v for k, v in self.static_vars.items()},
            atmos_vars={k: v[idx:(idx + 1)] for k, v in self.atmos_vars.items()},
            metadata=Metadata(
    ...
    def to(self, device: str | torch.device, non_blocking=False) -> "Batch":
        """Move the batch to another device."""
        return self._fmap(lambda x: x.to(device, non_blocking=non_blocking))
